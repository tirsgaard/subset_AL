...
def manifold_experiment(init_model_subset: callable,
    ...
    print(f"Subset Perfect error: {pred_errors_perfect:.3f}")
    wandb.finish()
    
    # Train model with uniform sampling of manifold_accuracy accuracy
    data_manifold_pred, X_manifold_index = model_subset.sample(global_budget, 
                                                               problem, 
                                                               ensure_subset=True, 
                                                               black_listed_indexes=index_subset.tolist(),
                                                               subset_chance=manifold_accuracy)
    data_manifold_pred = problem.label_manifold(data_manifold_pred, X_manifold_index)
    wandb.finish()
    
    # Sample points from the manifold
    model_global = init_model_global(f"manifold_accuracy_run_{run_id}", device)
    model_global.model.load_state_dict(model_global_parameters)  # Reset the global model to the initial state to ensure fair comparison
    model_global.fit(data_manifold_pred,)
    pred_errors_accuracy = model_global.test(test_data)
    print(f"Accuracy uniform error: {pred_errors_accuracy:.3f}")
    wandb.finish()
    ...
    return pred_errors_subset, pred_errors_perfect, pred_errors_accuracy, manifold_accuracy#, upper_errors_subset, #pred_error_uniform, pred_errors_AL_subset, pred_errors_AL#, data_manifold_pred, X_subset_pred_potential
...
if __name__ == "__main__":
    ...
    pred_error_perfect = np.array([res[1] for res in results])
    pred_errors_accuracy = np.array([res[2] for res in results])
    manifold_accuracy = np.array([res[3] for res in results])
    ...
    uniform_mean, uniform_bound = generate_uncertainty_estimate(pred_error_perfect)
    subset_upper_mean, subset_upper_bound = generate_uncertainty_estimate(pred_errors_accuracy)
    manifold_mean, manifold_bound = generate_uncertainty_estimate(manifold_accuracy)
    ...
    print(f"Manifold accuracy: {manifold_mean:.3f} +- {manifold_bound:.3f}")